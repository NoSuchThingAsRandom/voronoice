...
impl BoundingBox {
    ...
    pub fn is_inside(&self, point: &Point) -> bool {
        ...
        let horizonal_ok =
            (self.top_right.x - self.width() <= point.x) && (point.x <= self.top_right.x);
        // bottom.y <= point.y <= top.y
        let vertical_ok =
            (self.top_right.y - self.height() <= point.y) && (point.y <= self.top_right.y);
    ...
    #[inline]
    pub(crate) fn which_edge(
        &self,
        point: &Point,
    ) -> (BoundingBoxTopBottomEdge, BoundingBoxLeftRightEdge) {
        (
    ...
    pub(crate) fn intersect_line(&self, a: &Point, b: &Point) -> (Option<Point>, Option<Point>) {
        ...
        if c_x.abs() > 4. * std::f64::EPSILON {
            ...
            if right_y <= boundary_top && right_y >= boundary_bottom {
                f = Some(Point {
                    x: boundary_right,
                    y: right_y,
                });
            ...
            if left_y <= boundary_top && left_y >= boundary_bottom {
                g = Some(Point {
                    x: boundary_left,
                    y: left_y,
                })
        ...
        if c_y.abs() > 4. * std::f64::EPSILON {
            if c_x.abs() < 4. * std::f64::EPSILON {
                ...
                if a.x <= boundary_right && a.x >= boundary_left {
                    // and crosses box
                    return (
                        Some(Point {
                            x: a.x,
            ...
            if top_x <= boundary_right && top_x >= boundary_left {
                h = Some(Point {
                    x: top_x,
                    y: boundary_top,
                })
            ...
            if bottom_x <= boundary_right && bottom_x >= boundary_left {
                i = Some(Point {
                    x: bottom_x,
                    y: boundary_bottom,
                })
